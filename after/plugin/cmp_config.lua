...
cmp.setup{ -- Call setup function
    preselect = cmp.PreselectMode.None,  -- Disable preselect feature for completion
    ...
    mapping = cmp.mapping.preset.insert({ -- Define mapping for nvim-cmp
        ['<C-e>'] = cmp.mapping.close(),                     -- Set mapping for close completion-menu: Ctrl+e
        ['<C-f>'] = cmp.mapping.scroll_docs(4),              -- Set mapping for scroll docs if it visible up: Ctrl+f 
        ['<C-d>'] = cmp.mapping.scroll_docs(-4),             -- Set mapping for scroll docs if it visible down: Ctrl+d
        ['<C-Space>'] = cmp.mapping.complete(),              -- Set mapping for open completion-menu: Ctrl+Space
        ['<C-CR>'] = cmp.mapping.confirm({ select = true }), -- Set mapping for select item in completion: Ctrl+Enter
        ['<S-CR>'] = cmp.mapping.confirm({ select = true }), -- Set mapping for select item in completion: Shift+Enter
        ["<Tab>"] = cmp.mapping(function(fallback)           -- Set mapping function: Tab
            if cmp.visible() then                                                           -- If completion menu is visible then:
                cmp.select_next_item({ behaviour = cmp.SelectBehavior.Select })             -- Set mapping for select_next_item in completion: Tab
            elseif luasnip.locally_jumpable(1) then                                         -- Check if snippet is jumpable then:
                luasnip.jump(1)                                                             -- Expand snippet
            elseif has_words_before() then                                                  -- If have writed word before then:
                cmp.complete()                                                              -- Invoke completion
            else
                fallback()                                                                  -- If you are using vim-endwise, this fallback function will be behaive as the vim-endwise. 
            end                                                                             -- End if-else statement
        end, { "i", "s" }),                                             -- End mapping function <- This mapping only in insert mode and...
        ["<S-Tab>"] = cmp.mapping(function(fallback)                                        -- Set mapping function: Shift+Tab
            if cmp.visible() then                                                           -- If completion menu is visible then:
                cmp.select_prev_item({ behaviour = cmp.SelectBehavior.Insert })             -- Set mapping for select_prev_item in completion: Shift+Tab
            elseif luasnip.locally_jumpable(-1) then                                        -- Check if snippet is jumpable back then:
                luasnip.jump(-1)                                                            -- Jump Back!!!
            else
                fallback()                                                                  -- If you are using vim-endwise, this fallback function will be behaive as the vim-endwise.
            end                                                                             -- End if-else statement
        end, { "i", "s" }),                                             -- End mapping function <- This mapping only in insert mode and...
    }),
    sources = cmp.config.sources({ -- Define sources for completion
        -- ╭──────────────────────────────────────────────────────────────────────────────╮
        -- │                                Sources option                                │
        -- ╰──────────────────────────────────────────────────────────────────────────────╯
    ...
    formatting = { -- Define formatting nvim-cmp
        ...
        format = function(entry, item) -- A function to customize completion menu appearance.
            ...
            item.kind = string.format(' %s ', kind_icons[item.kind] or " ") -- This concatnates the icons with the name of the item kind -> single letter indicating the type of completion
...
-- ╰──────────────────────────────────────────────────────────────────────────────╯
cmp.setup.filetype('gitcommit', { -- Call setup for specific filetype.
    sources = cmp.config.sources({ -- Define sources for completion
...
cmp.setup.filetype('lua', { -- Call setup for specific filetype.
    sources = cmp.config.sources({ -- Define sources for completion
        { name = 'nvim_lsp', priority = 100 },
        { name = 'nvim_lua' },
        { name = 'luasnip' },
...
for index, command_type in pairs({'/', '?'}) do -- Create for-loop with list of search command
    cmp.setup.cmdline(command_type, { -- Call setup function for search
...
cmp.setup.cmdline(':',{ -- Call setup function for command mode
    mapping = cmp.mapping.preset.cmdline(),
...
vim.api.nvim_set_hl(0, 'CmpItemAbbrMatch', {fg = "#ff8f40"})                  -- CmpItemAbbrMatch: The search matching highlight nvim-cmp
