...
local telescope_custom_actions = {} -- Create telescope custom functions list
function telescope_custom_actions._multiopen(prompt_bufnr, open_cmd) -- Create custom telescope actions to handle multiopen
    local picker = action_state.get_current_picker(prompt_bufnr) -- Get current pick from current buffer
    local selected_entry = action_state.get_selected_entry()    -- Get selected entry from picker
    local num_selections = #picker:get_multi_selection()        -- Get number of selected in multi selection
    if not num_selections or num_selections <= 1 then -- If the user do multi select then
        actions.add_selection(prompt_bufnr) -- Add selection list into buffer
    end -- End if-else statement
    actions.send_selected_to_qflist(prompt_bufnr) -- Send previously selected list buffer into qflist
    vim.cmd("cfdo " .. open_cmd) -- open all qflist into tab
...
vim.keymap.set('n', '<leader>fu', require("telescope").extensions.undo.undo, {noremap = true, silent = true, desc = 'Open Undotree'})
